...
CAMERA_INDEX       = 0  # set to None to auto-detect first available camera
DEAD_ZONE_DEG      = 12
RELEASE_ZONE_DEG   = 6
SOFT_ZONE_DEG      = 25
FLIP_CAMERA        = True
...
def camera_help_text():
    system = platform.system()
    if system == "Windows":
        return (
            "  -> Windows: Settings > Privacy & security > Camera\n"
            "  -> Close other apps using the webcam (Teams, Zoom, etc.)\n"
            "  -> Try setting CAMERA_INDEX = 0 in the script"
        )
    if system == "Darwin":
        return "  -> macOS: System Settings > Privacy & Security > Camera"
    return "  -> Check camera permissions and that no other app is using it"
